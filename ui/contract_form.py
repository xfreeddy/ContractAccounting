...
class ContractFormDialog(QDialog):
    ...
    def save_contract(self):
        ...
        # Сборка данных договора для передачи в сервисный слой
        from datetime import date as dt
        contract_data = {
            "Наименование": self.title_input[1].text().strip(),
            "ТипДоговора": self.type_combo[1].currentText(),
            "Сумма": amount,
            "ДатаНачала": dt(
                self.start_date[1].date().year(),
                self.start_date[1].date().month(),
                self.start_date[1].date().day(),
            ),
            "ДатаОкончания": dt(
                self.end_date[1].date().year(),
                self.end_date[1].date().month(),
                self.end_date[1].date().day(),
            ),
            "Статус": "Активен",
        }
        counterparty_name = self.counterparty_input[1].text().strip()

        # Дополнительная валидация через бизнес-слой
        errors = self.service.validate_contract_data(contract_data)
        if errors:
            QMessageBox.warning(self, "Ошибка валидации", "\n".join(errors))
            return

        # Сохранение через сервисный слой (создаёт контрагента если нужно)
        ok = self.service.create_with_counterparty(contract_data, counterparty_name)
        if ok:
            QMessageBox.information(self, "Успех", "Договор успешно сохранён в базе данных!")
            self.accept()
        else:
            QMessageBox.critical(
                self,
                "Ошибка",
                "Не удалось сохранить договор в базе данных.\n"
